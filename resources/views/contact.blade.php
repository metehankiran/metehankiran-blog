        ...
        <div class="grid md:grid-cols-2 gap-8">
                ...
                <div class="rounded-xl border bg-card text-card-foreground shadow p-6">
                    <form class="space-y-4" method="POST" action="{{ route('contact.store') }}">
                        ...
                        <div class="space-y-2"><label for="name" class="text-sm font-medium">İsim</label><input
                                ...
                                id="name" name="name" placeholder="Adınız Soyadınız" value="{{ old('name') }}">
                            @error('name')
                        ...
                        <div class="space-y-2"><label for="email" class="text-sm font-medium">E-posta</label><input
                                ...
                                id="email" name="email" placeholder="ornek@email.com" value="{{ old('email') }}">
                            @error('email')
                                <p class="text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="space-y-2"><label for="subject" class="text-sm font-medium">Konu</label><input
                                class="flex h-10 w-full rounded-md border {{ $errors->has('subject') ? 'border-red-500' : 'border-input' }} bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                                id="subject" name="subject" placeholder="Mesajınızın konusu" value="{{ old('subject') }}">
                            @error('subject')
                                <p class="text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="space-y-2"><label for="message" class="text-sm font-medium">Mesaj</label>
                            <textarea
                                class="flex min-h-[80px] w-full rounded-md border {{ $errors->has('message') ? 'border-red-500' : 'border-input' }} bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                                id="message" name="message" placeholder="Mesajınızı buraya yazın..." rows="6">{{ old('message') }}</textarea>
                            @error('message')
                                <p class="text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div><button
            ...
            <div class="space-y-6">
                <div class="rounded-xl border bg-card text-card-foreground shadow p-4 flex items-center space-x-4">
                    ...
                    <div>
                        <h2 class="font-medium">E-posta</h2>
                ...
                <div class="rounded-xl border bg-card text-card-foreground shadow p-4 flex items-center space-x-4">
                    ...
                    <div>
                        <h2 class="font-medium">Telefon</h2>
                ...
                <div class="rounded-xl border bg-card text-card-foreground shadow p-4 flex items-center space-x-4">
                    ...
                    <div>
                        <h2 class="font-medium">Konum</h2>
                        <p class="text-sm text-muted-foreground">İstanbul, Türkiye</p>
                    </div>
                </div>
                <div class="rounded-xl border bg-card text-card-foreground shadow p-6">
                    <h2 class="font-medium mb-4">Sosyal Medya</h2>
